...
const LoginForm = () => {
  ...
  const {
    ...
    mode: 'onSubmit',
    defaultValues: { email: 'tranquangbao2003@gmail.com', password: 'Pass123@' },
    resolver: zodResolver(schema)
  })

  const { mutate, isPending } = useMutation({
    mutationFn: (payload: TSchema) => authApiRequest.loginToNextServer(payload),
    onSuccess: (data) => {
      addToast({
        color: 'success',
          ...
          <CardFooter>
            <div className='flex w-full items-center justify-center'>
              <div className='text-sm text-ct-blue md:text-base'>
                ...
                <Link className='ml-1 font-medium text-ct-purple' href={APP_ROUTES.AUTH.REGISTER}>
                  {t('register')}
                </Link>
              </div>{' '}
            </div>
